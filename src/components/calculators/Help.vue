...
<template>
    <CalculatorWrapper :title="$t('menu.help')"
    ...
    >
        <div class="px-4 pt-1 pb-3 text-sm flex flex-col space-y-3">
            ...
            <p class="font-semibold">{{$t('help.issues')}}</p>
            <p class="whitespace-pre-wrap">{{$t('help.issues_text')}}</p>
            <p class="whitespace-pre-wrap" v-if="!['en','nl'].includes($i18n.locale)">{{$t('translation')}}: {{$t('translator')}}</p>
        </div>
    </CalculatorWrapper>
</template>
