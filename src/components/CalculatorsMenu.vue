<script setup>
    ...
    const menu = [
        ...
        },
        {
            ...
            // icon: '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-8 h-8"><path stroke-linecap="round" stroke-linejoin="round" d="M2.25 18L9 11.25l4.306 4.307a11.95 11.95 0 015.814-5.519l2.74-1.22m0 0l-5.94-2.28m5.94 2.28l-2.28 5.941" /></svg>\n',
            calculator: {
...
<template>
    ...
    <div v-show="deletePopUp" class="absolute -left-2 -right-2 bottom-2 bg-white border rounded shadow-md">
        <div class="flex flex-col gap-3 px-2 py-4 text-sm">
            <p class="text-sm">Are you sure you want to clear the tab?</p>
            <button class="px-3 pt-0.5 pb-1 w-full font-semibold hover:opacity-90 bg-red-600 rounded" @click="deletePopUp = false; $emit('clear:tab')">yes</button>
            <button class="px-3 pt-0.5 pb-1 w-full font-semibold hover:opacity-90 bg-emerald-600 rounded" @click="deletePopUp = false">no</button>
        </div>
    </div>
</template>
